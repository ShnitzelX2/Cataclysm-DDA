...
static const item_group_id itemgroup_test_edevices_standard( "test_edevices_standard" );
static const item_group_id itemgroup_test_edevices_power( "test_edevices_power" );
static const item_group_id itemgroup_test_edevices_compat( "test_edevices_compat" );
static const item_group_id itemgroup_test_edevices_incompat( "test_edevices_incompat" );
                           ...
                           std::vector<item_location> &copiable_efile_locs )
{
    ...
    for( item_location &edevice : edevice_locs ) {
        for( item *efile : edevice->efiles() ) {
            efile_locs.emplace_back( item_location( edevice, efile ) );
        }
    }
...
TEST_CASE( "edevice", "[activity][edevice]" )
{
    ...
    auto do_activity = [&]( item_location & used_edevice, std::vector<item_location> target_edevices,
    std::vector<item_location> selected_efiles, efile_action action, bool pass_time = true ) {
        efile_activity_actor act( used_edevice, target_edevices, selected_efiles, action );
        dummy.assign_activity( act );
        process_activity( dummy, pass_time );
    };

    auto add_edevices = [&]( item_group_id igroup, bool add_etd = true ) {
        dummy.clear_worn();
        dummy.wear_item( item( itype_test_backpack ) );
        edevice_locs.clear();
        //this item group has two entries: the first is a laptop with two non-copiable files and one copiable file,
        //and the last is a tablet with no files
    ...
    SECTION( "move to / move from" ) {
        add_edevices( itemgroup_test_edevices_standard );
        REQUIRE( laptop_with_files->efiles().size() == 3 );
        REQUIRE( edevice_without_files->efiles().empty() );
        do_activity( edevice_without_files, { laptop_with_files }, efile_locs, EF_MOVE_ONTO_THIS );
        ...
        do_activity( edevice_without_files, { laptop_with_files }, efile_locs, EF_MOVE_FROM_THIS );
        REQUIRE( laptop_with_files->efiles().size() == 3 );
        REQUIRE( edevice_without_files->efiles().empty() );
    }
    SECTION( "copy onto" ) {
        add_edevices( itemgroup_test_edevices_standard );
        do_activity( edevice_without_files, { laptop_with_files }, copiable_efile_locs, EF_COPY_ONTO_THIS );
        REQUIRE( laptop_with_files->efiles().size() == 3 );
    ...
    SECTION( "copy from" ) {
        add_edevices( itemgroup_test_edevices_standard );
        do_activity( laptop_with_files, { edevice_without_files }, copiable_efile_locs, EF_COPY_FROM_THIS );
        REQUIRE( laptop_with_files->efiles().size() == 3 );
        REQUIRE( edevice_without_files->efiles().size() == 1 );
        REQUIRE( edevice_without_files->efiles().front()->typeId() == itype_test_efile_copiable );
        item *copied_efile = laptop_with_files->get_item_with( copy_efile_filter );
    ...
    SECTION( "wipe" ) {
        add_edevices( itemgroup_test_edevices_standard );
        do_activity( edevice_without_files, { laptop_with_files }, efile_locs, EF_WIPE );
        REQUIRE( edevice_without_files->efiles().empty() );
        REQUIRE( laptop_with_files->efiles().empty() );
    }
    SECTION( "empty parameters test" ) {
        ...
        efile_activity_actor act( empty_loc, { empty_loc }, efile_locs, EF_MOVE_ONTO_THIS );
        dummy.assign_activity( act );
        dummy.activity.do_turn( dummy );
    ...
    SECTION( "move to used edevice runs out of power" ) {
        add_edevices( itemgroup_test_edevices_power );
        do_activity( edevice_without_files, { laptop_with_files }, efile_locs, EF_MOVE_ONTO_THIS );
    ...
    SECTION( "move from target edevice runs out of power" ) {
        add_edevices( itemgroup_test_edevices_power );
        REQUIRE( edevice_without_files );
        do_activity( laptop_with_files, { edevice_without_files }, efile_locs, EF_MOVE_FROM_THIS );
    ...
    SECTION( "move to used edevice disappears" ) {
        add_edevices( itemgroup_test_edevices_power );
        REQUIRE( edevice_without_files );
        efile_activity_actor act( edevice_without_files, { laptop_with_files }, efile_locs,
                                  EF_MOVE_ONTO_THIS );
        dummy.assign_activity( act );
        for( int i = 0; i < 20; i++ ) {
    ...
    SECTION( "move from target edevice disappears" ) {
        add_edevices( itemgroup_test_edevices_power );
        efile_activity_actor act( laptop_with_files, { edevice_without_files }, efile_locs,
                                  EF_MOVE_FROM_THIS );
        dummy.assign_activity( act );
        for( int i = 0; i < 20; i++ ) {
            calendar::turn += 1_seconds;
    ...
    SECTION( "fast move between compatible devices" ) {
        add_edevices( itemgroup_test_edevices_compat );
    ...
    SECTION( "slow move between incompatible devices" ) {
        add_edevices( itemgroup_test_edevices_incompat, false );
        time_point before = calendar::turn;
        do_activity( laptop_with_files, { edevice_without_files }, efile_locs, EF_MOVE_FROM_THIS );
    ...
    SECTION( "fast move between incompatible devices with ETD" ) {
        add_edevices( itemgroup_test_edevices_incompat );
        time_point before = calendar::turn;
        do_activity( laptop_with_files, { edevice_without_files }, efile_locs, EF_MOVE_FROM_THIS );
