    ...
    if( not_browsing ) {
        ...
        for( item_location edevice : processed_edevices_locs ) {
            ...
            for( item *efile : efile_ptrs ) {
                selected_efiles.emplace_back( item_location( edevice, efile ) );
            }
        }
    }
...
std::optional<int> iuse::camera( Character *p, item *it, const tripoint_bub_ms & )
{
    ...
    item *edevice_photos = it->get_contents().get_item_with( filter );
    ...
    if( c_shot == choice ) {
        if( edevice_photos == nullptr ) {
            if( room_for_photo ) {
                ...
                edevice_photos = it->get_contents().get_item_with( filter );
            } else {
...
std::optional<int> iuse::view_photos( Character *p, item *it, const tripoint_bub_ms &pos )
{
...
std::optional<int> iuse::view_recipes( Character *p, item *it, const tripoint_bub_ms &pos )
{
    p->mod_moves( -to_moves<int>( 1_seconds ) * 0.5 );
