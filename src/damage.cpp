...
void damage_instance::add( const damage_unit &added_du )
{
    ...
    if( iter == damage_units.end() ) {
        ...
        if( added_du.barrels.empty() ) {
            ...
            //adding two sets of barrel definitions for damage together is not currently supported
        }
    }
}
...
void damage_instance::handle_proportional( const JsonObject &jo )
{
